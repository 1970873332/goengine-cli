...
function cloneRepo(options: Repository): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
        ...
        const gitProcess = spawn("git", args, { stdio: "inherit", cwd: targetDir });
...
const checkGit = () => {
    try { return execSync('git --version', { stdio: 'ignore' }), true; }
    catch { return false; }
};

if (!checkGit()) throw new Error("❌ Git is not installed or not available in PATH");

const results = await Promise.allSettled(
    config.repositories.map(cloneRepo)
);
